...
class ComponentsHandler {
    ...
    load = () => {
        const componentsPath = require('path').join(__dirname, '../../components');
        for (const directory of readdirSync(componentsPath)) {
            const directoryPath = require('path').join(componentsPath, directory);
            for (const file of readdirSync(directoryPath).filter((f) => f.endsWith('.js'))) {
                try {
                    /**
