...
class EventsHandler {
    ...
    load = () => {
        let total = 0;
        const eventsPath = require('path').join(__dirname, '../../events');
        
        for (const directory of readdirSync(eventsPath)) {
            const directoryPath = require('path').join(eventsPath, directory);
