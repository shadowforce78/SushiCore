...
module.exports = new MessageCommand({
    ...
    run: async (client, message, args) => {
        ...
        if (args[0] === config.commands.prefix) {
            await client.database.delete(`prefix-${message.guild.id}`, { guildId: message.guild.id });
        } else {
            await client.database.set(`prefix-${message.guild.id}`, args[0], { guildId: message.guild.id });
        }
